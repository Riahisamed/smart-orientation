...
export class StudentService {
  ...
  calculateT(student: any, formula: string) {
    ...
    const variables: Record<string, number> = {
      FG: student.FG || 0,
      ANG: student.english || 0,
      AR: student.arabic || 0,
      M: student.math || 0,
      F: student.french || 0,
      ESP: student.spanish || 0,
      IT: student.italian || 0,
      ALL: student.german || 0,
      HG: student.historyGeo || 0,
      PH: student.philosophy || 0,
      SP: student.physics || 0,
      INFO: student.algo || 0,
      TECH: student.tech || 0,
      ECON: student.economy || 0,
      GESTION: student.gestion || 0,
      SVT: student.svt || 0,
      TE: student.tech || 0,
      INF: student.algo || 0,
      SPT: student.spSport || 0,
      FR: student.french || 0,
    };

    try {
      let result = this.evaluateFormula(formula, variables);
  ...
  // حساب chance
  //////////////////////////////
  private evaluateFormula(formula: string, variables: Record<string, number>) {
  ...
  testStudentChances(student: any, filieres: any[], limit?: number) {
    ...
    const results = validFilieres.map((filiere) => {
      ...
      const chance = this.calculateChance(studentScore, lastScore);
    ...
    const uniqueResults = results.filter(
      (f, index, self) => index === self.findIndex((x) => x.code === f.code),
    );
