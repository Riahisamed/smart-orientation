...
export class AiService {
  ...
  async generateWithContext(
    ...
    const input: AiGenerateInput = {
      message,
      language,
      systemInstruction: this.buildStrictSystemInstruction(
        language,
        studentData,
        questionType,
      ),
  ...
  private buildDeterministicResponse(
    ...
    switch (questionType) {
      ...
      case 'programs':
        return this.buildProgramsResponse(
          ragData,
          studentData,
          language,
          followUp,
      ...
      case 'advice':
        return this.buildAdviceResponse(
          ragData,
          studentData,
          language,
          followUp,
      ...
      case 'general':
        return this.buildGeneralContextResponse(
          ragData,
          studentData,
          language,
          followUp,
  ...
  private buildJobsResponse(
    ...
    if (!jobs.length) {
      return this.withFollowUp(
        [
          language === 'ar'
            ? 'Ma andich jobs mouath9in lel domaine hedha.'
            : 'Pas de jobs verifies dans le contexte.',
        ],
        followUp,
      );
    }
    ...
    const lines = jobs.map((job) => {
      const description = this.shortText(
        job.description ||
  ...
  private buildProgramsResponse(
    ...
    if (!programs.length) {
      return this.withFollowUp(
        [
          language === 'ar'
            ? 'Ma andich programs mouath9in lel score hedha.'
            : 'Pas de programmes verifies dans le contexte.',
        ],
        followUp,
      );
    }
    ...
    const lines = programs.map((program) => {
      ...
      const scoreText =
        typeof program.lastScore === 'number' ? program.lastScore : 'N/A';
  ...
  private buildAdviceResponse(
    ...
    if (!decisions.length) {
      return this.withFollowUp(
        [
          language === 'ar'
            ? 'Ma najemch na9arrer bla programs mouath9in.'
            : 'Je ne peux pas décider sans programmes vérifiés.',
        ],
        followUp,
      );
    }
  ...
  private buildComparisonResponse(
    ...
    if (ragData.field) {
      const bits = [
        ...
        ragData.field.unemploymentRisk
          ? `chomage ${ragData.field.unemploymentRisk}`
          : undefined,
        ragData.field.outlook,
      ].filter(Boolean);
      return this.withFollowUp(
        [bits.join(' | ') || 'Comparaison impossible avec le contexte actuel.'],
        followUp,
      );
    }
    ...
    return this.withFollowUp(
      [
        language === 'ar'
          ? 'Ma andich zouz options mouath9in bech n9aren.'
          : 'Il manque deux options verifiees pour comparer.',
      ],
      followUp,
    );
  }
  ...
  private buildRoadmapResponse(
    ...
    const domain =
      studentData.memory?.refinedDomain ||
    ...
    if (language === 'ar') {
      ...
      lines.push(
        `2. Prochaine étape: ${this.roadmapNextStep(domain, language)}`,
      );
  ...
  private buildGeneralContextResponse(
    ...
    if (ragData.jobs?.length) {
      return this.withFollowUp(
        [
          language === 'ar'
            ? 'Najem naawnek b jobs, programs wala decision.'
            : 'Je peux cadrer jobs, programmes ou decision.',
        ],
        followUp,
      );
    }
    ...
    if (ragData.field && !['jobs', 'programs'].includes(questionType)) {
      parts.push(
        [
          ...
          ragData.field.unemploymentRisk
            ? `Unemployment risk: ${ragData.field.unemploymentRisk}`
            : undefined,
          ragData.field.outlook
  ...
  private filterAndRankPrograms(
    ...
    if (discussed.length > 0) {
      programs = programs.filter(
        (program) => !discussed.includes(program.name),
      );
    }
  ...
  private generateComparison(
    ...
    if (!best || !backup) {
      return language === 'ar'
        ? 'Decision: khoud el option elli riskha a9al.'
        : 'Decision: prends le risque le plus bas.';
    }
  ...
  private roadmapNextStep(
    ...
    if (this.hasAny(normalized, ['sport', 'coaching', 'kine'])) {
      return language === 'ar'
        ? 'pratique + stage fi club/clinique'
        : 'pratique + stage club/clinique';
    }
    if (this.hasAny(normalized, ['business', 'finance', 'marketing'])) {
      return 'case studies + Excel/analytics practice';
    }
    return language === 'ar'
      ? 'ebda bel basics, baad project sghir'
      : 'bases d abord, puis petit projet';
  }

  private roadmapProof(
    domain: string | undefined,
    language: AiLanguage,
  ): string {
    const normalized = this.normalizeForIntent(domain || '');
    ...
    return language === 'ar'
      ? 'project sghir + certificate mouath9a'
      : 'petit projet + certificat verifie';
  }
  ...
  private getFollowUpQuestion(
    ...
    const domain =
      this.detectDomainFromText(message) ||
      memory?.refinedDomain ||
      memory?.interest ||
      memory?.preferredDomain;
    const candidates = this.followUpCandidates(
      domain,
      memory,
      questionType,
      language,
    ...
    const fresh = candidates.find(
      (question) => !this.wasAsked(question, asked),
    );
    return (
      fresh ||
      candidates[0] ||
      (language === 'ar'
        ? ' تحب نثبتو الاختيار؟'
        : 'Tu veux trancher quelle option ?')
    );
  }
  ...
  private wasAsked(question: string, askedQuestions: string[]): boolean {
    const normalized = this.normalizeForIntent(question)
      .replace(/\s+/g, ' ')
      .trim();
    return askedQuestions.some((asked) => {
      const normalizedAsked = this.normalizeForIntent(asked)
        .replace(/\s+/g, ' ')
        .trim();
      return (
        normalizedAsked.includes(normalized.slice(0, 12)) ||
        normalized.includes(normalizedAsked.slice(0, 12))
      );
    });
  }
  ...
  extractStateFromMessage(
    ...
    if (
      ...
      if (this.hasAny(normalized, ['reseau', 'reseaux', 'network'])) {
        ...
        if (!newState.rejectedTopics.includes('reseaux'))
          newState.rejectedTopics.push('reseaux');
      }
      if (this.hasAny(normalized, ['math', 'mathematiques'])) {
        if (!newState.rejectedTopics) newState.rejectedTopics = [];
        if (!newState.rejectedTopics.includes('math'))
          newState.rejectedTopics.push('math');
      }
      if (this.hasAny(normalized, ['dev', 'developpement'])) {
        if (!newState.rejectedTopics) newState.rejectedTopics = [];
        if (!newState.rejectedTopics.includes('dev'))
          newState.rejectedTopics.push('dev');
      }
    }
  ...
  private detectDomainFromText(message: string): string | undefined {
    ...
    if (this.hasAny(normalized, ['health', 'sante', 'medecine', 'medical']))
      return 'health';
    if (
      this.hasAny(normalized, [
        'business',
      ...
      return 'business';
    if (this.hasAny(normalized, ['art', 'design', 'architecture']))
      return 'art';
    return undefined;
  }

  private formatMemory(memory: AiMemory, language: AiLanguage): string {
    const rejected = memory.rejectedDomains?.length
      ? memory.rejectedDomains.join(', ')
      : 'none';
    const asked = memory.lastQuestionsAsked?.length
      ? memory.lastQuestionsAsked.slice(-3).join(' | ')
      : 'none';
    return [
  ...
  private enforceResponseShape(text: string, input: AiGenerateInput): string {
    ...
    const lines = (text || '')
      ...
      .filter((line) =>
        this.isAllowedForType(line, input.questionType || 'general'),
      )
    ...
    const body = lines.length
      ...
      : [
          input.language === 'ar'
            ...
            : 'Je peux aider avec les donnees verifiees.',
        ];
  ...
  private hasAny(value: string, keywords: string[]): boolean {
    return keywords.some((keyword) =>
      value.includes(this.normalizeForIntent(keyword)),
    );
  }
        ...
        config.url,
        {
          ...
          options: {
            ...
            num_predict: Math.min(
              Math.max(config.num_predict ?? 180, 120),
              220,
            ),
            top_p: 0.7,
          },
