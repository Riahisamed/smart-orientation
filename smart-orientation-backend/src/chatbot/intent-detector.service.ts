...
export class IntentDetectorService {
  detectIntent(message: string): AdvisorIntent {
    ...
    // ============================================================
    if (
      /ما نحبش|مانحبش|manhebch|ma nhebch|ma n7ebch|موش|مش/i.test(normalized)
    ) {
      return 'rejection';
    }

    // ============================================================
    // 2. GREETING — only if nothing substantive after the hello
    //    (e.g. "salem n7eb domaine..." must not be swallowed as greeting)
    // ============================================================
    const greetingLead =
      /^(salut|bonjour|hello|hi|hey|صباح|مساء|سلام|salem|سلام\s+عليكم|بienvenue|اهلا|أهلا)\b/i;
    if (greetingLead.test(normalized)) {
      const afterHello = normalized
        .replace(greetingLead, '')
        .replace(/^[,!\s]+/, '')
        .trim();
      if (!afterHello || afterHello.length < 6) {
        return 'greeting';
      }
    }
    ...
    if (
      /\b(score|moyenne|fg)\b.*\b(chnowa|najem|faire|choisir|orientation|programme|filiere)\b/i.test(
        normalized,
      )
    ) {
      return 'ask_programs';
  ...
  private isComparison(text: string): boolean {
    ...
    // خير بين / أفضل بين
    if (
      /(خير\s+بين|أفضل\s+بين|افضل\s+بين|a7sen\s+bin|ahsen\s+bin|meilleur\s+entre)/i.test(
        text,
      )
    )
      return true;
  ...
  private isJobQuestion(text: string): boolean {
    ...
    if (
      /\b(tell\s+me\s+about|parle[-\s]?moi\s+(du|de\s+la|des?)|décris|décrire|describe)\s+/i.test(
        text,
      )
    )
      return true;
    ...
    if (
      /(c[']est\s+quoi|c[']est\s+comment|qu[']est-ce\s+que)\s+(le\s+)?(m[ée]tier|travail|job|profession)/i.test(
        text,
      )
    )
      return true;
    ...
    if (
      /(data\s+scientist|cyber|cybersécurité|développeur|developpeur|web\s+developer|fullstack|data\s+analyst|data\s+engineer|devops)\s*(c[']est|هو|hiya|شنو|شنوا|اشنو)/i.test(
        text,
      )
    )
      return true;
  ...
  private isProgramQuestion(text: string): boolean {
    ...
    if (
      /شنو[ّاا]?\s+(نقرا|ندير|نعمل|نختار|نحط|نعيش|programme|filiere|faculté|université)/i.test(
        text,
      )
    )
      return true;
    ...
    if (
      /شنو[ّاا]?\s+(الجامعة|البرنامج|التخصص|الفيلار|المجال|lprogramme|filiere|specialite|fac)/i.test(
        text,
      )
    )
      return true;
    ...
    if (
      /أي\s+(تخصص|fac|fili[èe]re|universit[ée]|programme|مدرسة|جامعة)/i.test(
        text,
      )
    )
      return true;
    if (
      /اي\s+(تخصص|fac|fili[èe]re|universit[ée]|programme|مدرسة|جامعة)/i.test(
        text,
      )
    )
      return true;
    ...
    if (
      /نحب\s+(informatique|sport|طب|صحة|تجارة|فن|رياضة|تكنولوجيا|انفورماتيك|معلومية|اعلامية)/i.test(
        text,
      )
    )
      return true;
    if (
      /nheb\s+(info|informatique|informatia|sport|tech|it|medecine|commerce|art)/i.test(
        text,
      )
    )
      return true;
    if (
      /n7eb\s+(info|informatique|informatia|sport|tech|it|medecine|commerce|art|domaine|برنامج|تخصص)/i.test(
        text,
      )
    )
      return true;
    ...
    if (/(شنو|شنوا)?\s*(تنصحني|nansahni|ansahni|نصحني)/i.test(text))
      return true;

    // French choosing patterns
    if (
      /(que\s+)?(choisir|programme|fili[èe]re|quel\s+domaine|quel\s+programme)/i.test(
        text,
      )
    )
      return true;
    if (
      /je\s+(veux|voudrais|cherche)\s+(choisir|faire|un\s+programme|une\s+fili[èe]re)/i.test(
        text,
      )
    )
      return true;
    if (
      /j[']aime\s+(l[']informatique|le\s+sport|la\s+sant[ée]|les\s+affaires)/i.test(
        text,
      )
    )
      return true;
    ...
    if (domainTerms.some((t) => text.includes(t))) {
      ...
      if (
        !this.isJobQuestion(text) &&
        !this.isRoadmapQuestion(text) &&
        !this.isSkillsQuestion(text)
      ) {
        return true;
      }
    }
